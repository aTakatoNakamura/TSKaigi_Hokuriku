...
TSはサーバーでもフロントでも使用していて、ts歴は７年くらいになりました。
たまにアマチュアオーケストラでバイオリン弾いてます。
-->
...
従来の書き方だと、内側から外側へ実行されるので、
実際の処理順がコードの見た目と逆になり、とても読みづらいです。

パイプ演算子を使うと、左から右へ処理の流れが読めるので、
コードの見た目と実行順が一致して、とても理解しやすくなります。

ちなみに、TC39でも提案されていて、現在はstage2なので、そのうちjsでもサポートされるかもしれないですね。
...
今回実装したものは [aTakatoNakamura/TypeScript](https://github.com/aTakatoNakamura/TypeScript) にあります。
